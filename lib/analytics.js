...
Analytics.prototype.identify = function (id, traits, options, fn) {
  ...
  // call
  this._invoke('identify', identify);
...
Analytics.prototype.group = function (id, properties, options, fn) {
  ...
  // group
  var facade = new Group({
    properties: properties,
    options: options,
    groupId: id
  });

  this._invoke('group', facade);
  this.emit('group', id, properties, options);
  this._callback(fn);
  return this;
};
 ...
 * TODO: refactor when facade is ready
 *
 * @param {String} method
 * @param {Mixed} args...
 * @return {Analytics}
...
Analytics.prototype._invoke = function () {
  ...
  // TODO: no need to clone, take options from facade.
  ...
  // TODO: remove me
  if ('identify' == args[0]) return;
  if ('group' == args[0]) return;
  if ('track' == args[0]) return;
  if ('alias' == args[0]) return;
  if ('page' == args[0]) return;
