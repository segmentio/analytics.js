...
describe('#defaults', function () {
  ...
  it('loggingLevel', function () {
    assert(this.integration.defaults.loggingLevel === 'NONE');
  });

  it('minified', function () {
    assert(this.integration.defaults.minified === true);
  });
});
...
describe('#initialize', function () {
  ...
  it('should store options', function () {
    ...
    assert(this.options.loggingLevel == settings.loggingLevel);
    assert(this.options.minified == settings.minified);
  });
  ...
  it('should show a message', function (done) {
    when(function () { return ajq('.evergage-qtip').length && ajq('.evergage-qtip').css('opacity') == 1; }, done);
  });
...
describe('#identify', function () {
  ...
  it('shouldn\'t send just traits', function () {
...
describe('#group', function () {
  ...
  it('should send properties', function () {
    ...
    // Analytics.js now always caches the group id and always sends it
    // assert(!this.stub.calledWith(['setCompany', 'id']));
  });
