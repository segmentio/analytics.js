...
/**
 * Add `fn` hook with `type` or invoke all `type` hooks with `args`.
 *
 * @param {String} type
 * @param {Function} fn
 * @return {Analytics}
...
Analytics.prototype.hook = function(type, fn){
  if ('function' == typeof fn) {
    this.log('hook - %s', type);
    (this.hooks[type] = this.hooks[type] || []).push(fn);
    return this;
  }
  ...
  this.log('hook %o - %o', type, hooks.length);
  for (var i = 0, fn; fn = hooks[i++];) {
    ret = fn.apply(this, args);
  }

  this.log('hook %o - %o', type, ret);
  return ret;
};
...
Analytics.prototype.initialize = function (settings, options) {
  ...
  this.log('initialize - %o', arguments);
...
Analytics.prototype.setAnonymousId = function(id){
  this.log('set anonymous id - %o', id);
...
Analytics.prototype._invoke = function (method, facade) {
  ...
  each(this._integrations, function (name, integration) {
    ...
    if (self.hook('ignore', [integration, facade])) return;
    debug('invoke %o .%s() %o', name, method, facade);
