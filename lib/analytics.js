...
Analytics.prototype.page = function (section, name, properties, options, fn) {
  if (is.fn(options)) fn = options, options = undefined;
  if (is.fn(properties)) fn = properties, properties = undefined;
  if (is.fn(name)) fn = name, name = section, section = undefined;
  if (is.object(name)) {
    if (is.string(section)) {
      options = properties, properties = name, name = section;
      section = undefined;
    } else {
      options = name, properties = section, name = section = undefined;
    }
  }

  if (is.string(properties)) properties = { path: properties };
  if (is.string(section) && !name) name = section, section = undefined;
